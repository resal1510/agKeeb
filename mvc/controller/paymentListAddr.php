...
foreach ($resultAddr1 as $key) {
  ...
  if ($isDefault) {
    echo '<div id="addressL'.$idAddress.'" class="col-auto p-2 border-custom-selected mb-3 rounded customDivAddress"><span>'.$name.' '.$surname.'<br>'.$address.'<br>'.$npa.' '.$city.'</span></div>';
  } else {
    echo '<div id="addressL'.$idAddress.'" class="col-auto p-2 border-custom mb-3 rounded customDivAddress"><span>'.$name.' '.$surname.'<br>'.$address.'<br>'.$npa.' '.$city.'</span></div>';
  }
  $number++;
}
?>
